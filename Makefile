...
.PHONY: build
build: types shared forces testing postgres-gateway csv-to-xlsx \
...
src/lambdas/automation-report/build: $(SHARED_SOURCE) $(FORCES_SOURCE) $(TYPES_SOURCE) $(AUTOMATION_REPORT_SOURCE)
	cd src/lambdas/automation-report && npm run build

src/lambdas/common-platform-report/build: $(TYPES_SOURCE) $(COMMON_PLATFORM_REPORT_SOURCE)
