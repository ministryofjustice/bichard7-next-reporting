import type { ApiConfig, AuditLog, Interval, PromiseResult } from "@bichard/types"
...
const fetchReportRecords = async (report: string, interval: Interval, config: ApiConfig): Promise<AuditLog[]> => {
  ...
  while (true) {
    const fetchResult: AuditLog[] = await fetchReportRecordsPage(report, interval, config, lastMessageId)
